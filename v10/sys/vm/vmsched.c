...
setupclock()
{
	...
	if (slowscan == 0)
		slowscan = 2 * fastscan;
	vmpago();	/* to fire off timeout */
	timeout(vmmeter, (caddr_t)0, 11);	/* offset from 1-sec clock uproar */
}
...
vmmeter()
{
	...
	timeout(vmmeter, (caddr_t)0, HZ);
}

#define	PAGOPSEC	4	/* how often we are called per second */
...
vmpago()
{
	...
	timeout(vmpago, (caddr_t)0, HZ/PAGOPSEC);
	/*
