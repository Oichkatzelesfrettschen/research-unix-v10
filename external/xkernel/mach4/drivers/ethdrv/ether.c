...
#define checkResult(_r, _msg)						\
	...
	sprintf(errBuf, "%s fatal initialization error: %s (%s)",	\
		self->name, (_msg), mach_error_string(kr));		\
	xError(errBuf);							\
     ...
     XObj self;
{
    ...
    kr = device_open(task_by_pid(-2),0,state->devname,&state->interface_port);
    checkResult(D_SUCCESS, "failed to get device port");
