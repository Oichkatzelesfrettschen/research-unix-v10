...
#
# HISTORY
# 28-Jun-93  Mary Thompson (mrt) at Carnegie-Mellon University
#	Added rule for mach_norma_user.c and mach_norma.h
#
# $Log: template.mk,v $
# Revision 2.9.1.2  1993/11/11  22:22:52  menze
# xkernel include directories were split into separate searchable and
# non-searchable lists
#
# Revision 2.9.1.1  1993/09/23  00:37:54  menze
...
XK_GEN 		= xkern.gen
MKODIRS		= mach/ mach_debug/ device/ $(XK_GEN)/
...
INCDIRS		:= $(INCDIRS) $(XK_INC)
INCFLAGS	= -I.. $(XK_INC_SEARCH)
...
genassym: xxx_genassym.c
	( LPATH="/usr2/mach3.82.sup/export/pmax_mach/uofa:${_host_LPATH_}"; export LPATH; \
...
uproxy.o: $(XK_GEN)/xk_lproxy.h



XK_UPROXY_FILES = $(XK_GEN)/xk_uproxyServer.c $(XK_GEN)/xk_uproxy.h

${XK_UPROXY_FILES} : xk_uproxy_stamp
xk_uproxy_stamp: xkern/mach3/api/proxy/xk_uproxy.defs
	@touch xk_uproxy_stamp
	@echo ""
	$(MIG) -MD $(MIGKSFLAGS) \
	    -user /dev/null \
	    -header $(XK_GEN)/xk_uproxy.h \
	    -server $(XK_GEN)/xk_uproxyServer.c \
		${xkern/mach3/api/proxy/xk_uproxy.defs:P}



XK_LPROXY_FILES = $(XK_GEN)/xk_lproxyUser.c $(XK_GEN)/xk_lproxy.h

${XK_LPROXY_FILES} : xk_lproxy_stamp
xk_lproxy_stamp: xkern/mach3/api/proxy/xk_lproxy.defs xkern/mach3/api/proxy/lproxy.awk xkern/mach3/api/proxy/lproxy.demux.sed
	@touch xk_lproxy_stamp
	@echo ""
	$(MIG) -MD $(MIGKUFLAGS) \
	    -user $(XK_GEN)/xk_lproxyUser.c.tmp \
	    -header $(XK_GEN)/xk_lproxy.h \
	    -server /dev/null \
		${xkern/mach3/api/proxy/xk_lproxy.defs:P} ; \
	awk -f ${xkern/mach3/api/proxy/lproxy.awk:P} $(XK_GEN)/xk_lproxyUser.c.tmp; \
	mv lproxy.rest.tmp $(XK_GEN)/xk_lproxyUser.c; \
	sed -f ${xkern/mach3/api/proxy/lproxy.demux.sed:P} lproxy.demux.tmp >> $(XK_GEN)/xk_lproxyUser.c; \
	rm lproxy.demux.tmp $(XK_GEN)/xk_lproxyUser.c.tmp; 
