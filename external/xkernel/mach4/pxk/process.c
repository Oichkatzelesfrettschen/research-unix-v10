     ...
     int priority;
{
  ...
  child = kernel_thread( kernel_task, enter_master_sledgehammer_monitor, 0 );
	...
	Semaphore		*s;
{
	...
	if (s->count < 0) {
		...
		xTrace1(processswitch, TR_GROSS_EVENTS, "realP wait_result: %d",
	...
	Semaphore		*s;
{
	...
	if( s->count <= 0 ) {
		...
		xTrace2(processswitch, TR_GROSS_EVENTS, "Resuming via V: %#x thread: 0x%x",
				  s, w->thread );
		clear_wait(w->thread, THREAD_AWAKENED, FALSE);
	}
#endif ! XKMACHKERNEL
}
