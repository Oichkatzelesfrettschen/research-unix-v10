 ...
 *
 * $Revision: 1.10 $
 * $Date: 1993/11/18 03:55:07 $
 * $Author: menze $
 *
 * $Log: xsi_services.c,v $
 * Revision 1.10  1993/11/18  03:55:07  menze
 * Adapted to change in semantics of GETPARTICIPANTS control op
 *
 * Revision 1.9  1993/06/28  18:42:03  ho
 * Made function inline compilation dependent on __GNUC__
 *
...
so_connect(socket_t *sd, struct sockaddr_in *peer, XObj *session)
{
	 ...
	 */
	{
	    ...
	    res = xControl(sd->so_session, GETPARTICIPANTS, partBuf,
			   sizeof(partBuf));
	    xAssert(res > 0);
	    xAssert(partExtLen(partBuf) < sizeof(p));
	       ...
	       struct sockaddr *name, int *namelen, int *errno)
{
	...
	*errno = EBADF;
	...
	res = xControl(sd->so_session, GETPARTICIPANTS, partBuf, sizeof(partBuf));
	xAssert(res > 0);
	xAssert(partExtLen(partBuf) < sizeof(p));
	partInternalize(p, partBuf);
    }
