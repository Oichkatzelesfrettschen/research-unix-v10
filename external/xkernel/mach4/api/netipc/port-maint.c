...
/* necassary because on some architectures the ip addr
   struct is passed to routines as a struct pointer instead
   of the the ip address itself */
#define IP_TO_INT(ip_addr) (*(int *)&(ip_addr))
...
void port_mgmt(mn_arch_tag_t arch, Msg *msg);

xkern_return_t portmaint_init(XObj self);

static void portm_port_free(ReadWriteLock *rwlock, mnetport *portdesc);
...
mach_port_t
portm_quick_so_lookup(mnportid_t portid);

xkern_return_t
portm_port_remove_send_once(mnetport *port_desc);

xkern_return_t
portm_port_remove_unused(mach_port_t *local_port, mnetport *netport_ptr,
  IPhost *recvr_ipaddr);
...
void
portm_add_sender(mnetport *portd, IPhost host);

void portm_delete_local_receive(Event ev, mach_port_t port);

void portm_delete_local_send(Event ev, mach_port_t port);

mnportid_t portm_get_netnum(mach_port_t port, IPhost sender);

mach_port_t portm_fast_send_once(mnportid_t nnum, IPhost receiver);

mnetport portm_move_netnum(mach_port_t port, mnportid_t netnum);
...
mnetport convert_to_netport(mach_port_t port, mach_port_type_t netright,
  IPhost dest, msg_id_t msgid, mnportid_t prev_netport_number);
...
xkern_return_t srxMoveSendRights(IPhost dst, MsgId msgId, mnetport **ports);

xkern_return_t rrxMoveReceiveRights(IPhost dst,MsgId msgId,mnetport **ports);
...
xkern_return_t portm_port_remove_unused();
...
xkern_return_t allocLocalRight();
		   ...
		   msg_id_t msgid, mnportid_t prev_netport_number)
{
  ...
  if (mapResolve(STATEPTR->maplocalports, &port, &netport) != XK_FAILURE) {
	...
	if (netright == MACH_PORT_TYPE_SEND) {
	  ...
	  port_array[1] = 0;
	  if (srxMoveSendRights(dest, msgid, (mnetport **)port_array)
	      == XK_FAILURE)
	    {
...
#ifdef PORTLOCKS
	  xTrace1(portmaintp, TR_EVENTS, "portmaint: convert_to_netport: Getting READER LOCK on port %x", netport->net_port_number);
	  ...
	  if (netright == MACH_PORT_TYPE_RECEIVE)
	    {
	      ...
	      port_array[0] = (mn_netport_t *)netport;
	      port_array[1] = 0;
	      if (rrxMoveReceiveRights(dest, msgid, (mnetport **)port_array)
		  == XK_FAILURE)
		{
		  xTrace1(portmaintp, TR_ERRORS, "portmaint: convert_to_netport could not move send right for port %x",
			  netport->net_port_number);
	...
	if (!(netport->net_port_rights & MACH_PORT_TYPE_RECEIVE))
	  {
	    ...
	    /* delete the send right that we had previously */
	    
	    if ((kret = 
		 mach_port_get_refs(mach_task_self(),
				    netport->real_send_port,
				    MACH_PORT_RIGHT_SEND,
				    (mach_port_urefs_t *)&refs))
		!= KERN_SUCCESS)
	      {
		xTrace2(portmaintp, TR_ERRORS, "portmaintp: convert_netport: could not get ref for port %x code %x",
			netport->real_send_port, kret);
	      }
	    xTrace2(portmaintp, TR_FULL_TRACE, "portmaintp: convert_netport: refs for port %x are %d", netport->real_send_port, refs);
	...
	if (cmpresult)
	  {
	    port_array[0] = (mn_netport_t *)netport;
	    port_array[1] = 0;
	    xTrace3(portmaintp, TR_EVENTS, "portmaintp: convert_to_netport: will move receive rights; netport %x: (readers = %d) (writers = %d)", netport->net_port_number, netport->rwlock.activeReaders, netport->rwlock.activeWriter);
	    if (rrxMoveReceiveRights(dest, msgid, (mnetport **)port_array)
		== XK_FAILURE)
	      {
		xTrace1(portmaintp, TR_ERRORS, "portmaint: convert_to_netport could not move receive right for port %x",
			netport->net_port_number);
		return Null_Netport;
	      }
  ...
  netport->lower_session = ERR_XOBJ;
  ...
  if ((ret = mapBind( STATEPTR->maplocalports, &port, netport)) != ERR_BIND) {
    ...
    if( (netright == MACH_PORT_TYPE_SEND && (rights & MACH_PORT_TYPE_SEND))
       || (netright == MACH_PORT_TYPE_SEND_ONCE &&
	   (rights & (MACH_PORT_TYPE_SEND_ONCE | MACH_PORT_TYPE_RECEIVE))))
      {
...
/*	netport->net_port_rights = MACH_PORT_TYPE_RECEIVE | MACH_PORT_TYPE_SEND;*/
	netport->net_port_rights = MACH_PORT_TYPE_SEND;
	netport->net_port_local_rights = MACH_PORT_TYPE_RECEIVE;
	netport->net_port_type = MN_VALID;  /* ??? */
	netport->real_receive_port = port;
	netport->receiver_host_addr = dest;
	netport->senders_map = mapCreate(INITIAL_MAX_MNIPC_SENDERS,sizeof(IPhost));
			     ...
			     msg_id_t msgid, IPhost sender)
{
  ...
  if (
      mapResolve(STATEPTR->mapnetdesc,
		 &(netport->net_port_number), &netport_ptr) == XK_SUCCESS)
    {
	/*
	 ...
	 */
        ...
        xTrace3(portmaintp, TR_EVENTS, "portmaintp: convert_to_netport: will move receive rights; netport %x: (readers = %d) (writers = %d)", netport_ptr->net_port_number, netport_ptr->rwlock.activeReaders, netport_ptr->rwlock.activeWriter);

        /* check if this right was being used by a previous unfinished
	   reply message the xCall of which is still pending */
        if (portm_quick_so_lookup(netport->net_port_number) != MACH_PORT_NULL
	    && netport_ptr->net_port_rights & MACH_PORT_TYPE_SEND_ONCE) {
	  portm_port_remove_send_once(netport_ptr);
	  xTrace1(portmaintp, TR_ERRORS, "portmaint: Removed Send Once right for a pending reply for netport id %x", netport->net_port_number);
	  goto new_port;
	}
...
/*	    (netport_ptr->net_port_local_rights & MACH_PORT_TYPE_RECEIVE)) { */
	  ...
	  if (netport_ptr->net_port_type != MN_WAITING) {
	    ...
	    return new_port_right; /* don't listen! */
	  }
	}
	if (netport->net_port_rights & MACH_PORT_TYPE_RECEIVE) {
	  ...
	  xTrace1(portmaintp, TR_FUNCTIONAL_TRACE, "got receive right port id %x",
		  netport->net_port_number);
	  xTrace3(portmaintp, TR_EVENTS, "portmaintp: convert_netport_to_mach_port: got a receive right; netport %x: (readers = %d) (writers = %d)", netport_ptr->net_port_number, netport_ptr->rwlock.activeReaders,netport_ptr->rwlock.activeWriter);
	  ...
	  /* if neither, we should not retain an entry for this port;
	     ...
	     wants it.
	   */
	  if (netport_ptr->senders_map == NULL) {
...
/*	  if (netport_ptr->sender_count) {
	  ...
	  /* HASNAIN: assume that old receiver is a sender, we will get an
	     nms notification from it, if it isnt */

	  netport_ptr->sender_count = 1;

	  /* use the make send count sent in the message i.e. of netport */
	  mapUnbind(netport_ptr->senders_map, &sender);
	  mapBind(netport_ptr->senders_map, &sender, netport->make_send_count);
...
/*      }  */
	...
	if (((netport->net_port_rights & MACH_PORT_TYPE_SEND) &&
					      ...
					      MACH_PORT_TYPE_SEND))))
	  {
	    ...
	    if (netport->net_port_rights & MACH_PORT_TYPE_SEND) {
	      /* need to have a map of all lower sessions to close them */
		...
		if ((ret = 
		    ...
		    != KERN_SUCCESS)
		  {
		    ...
		    if ((ret = mach_port_get_refs(mach_task_self(), netport_ptr->real_send_port, MACH_PORT_RIGHT_DEAD_NAME, &refs)) == KERN_SUCCESS) {
		      xTrace1(portmaintp, TR_ERRORS, "Number of dead refs = %d", refs);
		    }
		    if ((ret = mach_port_get_refs(mach_task_self(), netport_ptr->real_send_port, MACH_PORT_RIGHT_RECEIVE, &refs)) == KERN_SUCCESS) {
		      xTrace1(portmaintp, TR_ERRORS, "Number of receive refs = %d", refs);
		    }
		    return MACH_PORT_NULL;
		  }
	      }
	    ...
	    if (netport_ptr->senders_map == NULL)
	      netport_ptr->senders_map = mapCreate(INITIAL_MAX_MNIPC_SENDERS, sizeof(IPhost));
	    addNewSender(netport_ptr, sender, 0);

	    if (netport_ptr->net_port_type == MN_WAITING &&
		netport_ptr->msgid == msgid &&
		! bcmp((char *)&netport_ptr->orig_host, (char *)&sender,
		       sizeof(IPhost)))
...
new_port:
  ...
  if (mapResolve(STATEPTR->mapsendoncenumbers, &netport->net_port_number,
		 &new_port_right)
      == XK_SUCCESS) {
    xTrace2(portmaintp, TR_EVENTS, "portmaint: convert_netport_to_mach using rpc undo port, local name %x number %x",
	    new_port_right, netport->net_port_number);
    portm_use_netnum(new_port_right, netport->net_port_number);
    return new_port_right;
  }
  ...
  */
  ...
  netport_ptr->lower_session = ERR_XOBJ;
  ...
  semInit(&netport_ptr->listen_sem, 1);
 ...
 xTrace1(portmaintp, TR_EVENTS, "portmaint: convert_netport_to_mach absorbing new net port: local port = %x", netport_ptr->real_receive_port);
  ...
  xTrace1(portmaintp, TR_EVENTS, "portmaint: convert_netport_to_mach bound netport number %x", netport->net_port_number);
     ...
     Msg	*reply_msg;
{
  ...
  xTrace0(portmaintp, TR_EVENTS, "portmaint: complete_port_transfer");
				...
				IPhost rec_addr, msg_id_t msgid)
{
  ...
  xTrace4(portmaintp, TR_MORE_EVENTS, "convert_netport_to_tmp binding %x to structure at %x with local rights %x; receive port = %x",
...
/*
 *
 *  portm_quick_so_lookup()
 *
 */
mach_port_t
  portm_quick_so_lookup(portid)
    mnportid_t portid;
{
  mach_port_t mport;

  if (mapResolve(STATEPTR->mapsendoncenumbers, &portid, &mport) == XK_SUCCESS){
    return mport;
  }
  return MACH_PORT_NULL;
}
...
/*
	  ...
	  != KERN_SUCCESS) {
	xTrace2(portmaintp, TR_SOFT_ERRORS, "portmaint: port_mgmt: could not decrement port ref count on send port %d (x%x)", portdesc->real_send_port, portdesc->real_send_port);
	return;
      }
*/
...
/*    }*/
  ...
  xTrace1(portmaintp, TR_ERRORS, "portmaintp: port_mgmt unknown message type %d",
	  local_data.type);
}
     ...
     VOID *arg;
{
  ...
  mn_arch_tag_t arch_type = (mn_arch_tag_t)arg;
  Pfi convec[MAX_MACH_DATA_TYPE];

  set_convert_vector(arch_type, (Pfv *)convec);

  xTrace0(portmaintp, TR_FULL_TRACE, "portmaint: portm_unbundle called");
/*  msgstream += MN_ARCH_TAG_NETLEN + MACHIPCTYPE_NETLEN; */
  if (arch_type == MN_ARCH_MARKER) {
    ...
    bcopy(msgstream, (char *)&portstruct->net_port_number, PORTID_NETLEN);
    portstruct->net_port_number = (int)convec[PORTID_NETTYPE]((char *)&portstruct->net_port_number);
    portstruct->type = *(msgstream += PORTID_NETLEN);
    bcopy(msgstream += PORT_MGMT_TYPELEN, (char *)&portstruct->sender, HOSTNETLEN);
    bcopy(msgstream + HOSTNETLEN, (char *)&portstruct->make_send_count, NETMAKESENDSIZE);
    portstruct->make_send_count = (int)convec[NETMAKESENDTYPE]((char *)&portstruct->make_send_count);
  }
...
/*  if ( !( portd->net_port_rights & MACH_PORT_TYPE_RECEIVE ))*/
  if ( portd->net_port_rights & MACH_PORT_TYPE_SEND )
    {
	...
	if ( ! IPEQUAL(portd->receiver_host_addr, STATEPTR->local_source_addr))
	  {
	    ...
	    ls = xOpen(machnetipcProtl, machnetipcProtl,
		       xGetDown(machnetipcProtl, TRANSPORT_DNUM), whom);
     ...
     mnetport    *netport_ptr;
{
  ...
  if (mapUnbind(STATEPTR->mapnetdesc, &netport_ptr->net_port_number)
      != XK_SUCCESS) {
    /* already removed by an incoming message using the same port */
    xTrace1(portmaintp, TR_EVENTS, "portmaint: portm_port_remove_send_once netport %x has already been removed from maps", netport_ptr->net_port_number);
    return XK_SUCCESS;
  }
  else {
  ...
  if (netport_ptr->real_send_port != MACH_PORT_NULL) {
    xTrace1(portmaintp, TR_DETAILED, "portmaintp: remove_send_once: unbinding send_port %x", netport_ptr->real_send_port);
  ...
  if (netport_ptr->real_receive_port != MACH_PORT_NULL) {
    xTrace1(portmaintp, TR_DETAILED, "portmaintp: remove_send_once: unbinding receive_port %x", netport_ptr->real_receive_port);
    mapUnbind(STATEPTR->maplocalports, &netport_ptr->real_receive_port);
    if (mapUnbind(STATEPTR->mapsendonceports, &netport_ptr->real_receive_port)
	== XK_SUCCESS) {
      portm_use_fast_send_once(netport_ptr->real_receive_port, TRUE);
    }
    else
  ...
  xTrace1(portmaintp, TR_DETAILED, "portmaintp: remove_send_once: unbinding netport id %x", netport_ptr->net_port_number);
  portm_port_free(NULL, netport_ptr);
  return XK_SUCCESS;
}
     ...
     IPhost      *recvr_ipaddr;
{
  ...
  xTrace1(portmaintp, TR_EVENTS, "portm_port_remove_unused: removing id %x",
	  netport_ptr->net_port_number);
 ...
 */
static void
portm_dummy(rwlock, dummy) ReadWriteLock *rwlock; void *dummy;
{
  xTrace0(portmaintp, TR_FULL_TRACE, "port_maint: portm_dummy");
}
#endif PORTLOCKS
 ...
 */
static void
portm_port_free(rwlock, portdesc) ReadWriteLock *rwlock; mnetport *portdesc;
{
  ...
  bzero((char *)portdesc, sizeof(mnetport));
  xFree((char *)portdesc);
}
...
#ifdef PORTLOCKS
  rwLockDestroy(portdesc->rwlock_ptr, portm_dummy, (void *)0);
/*  rwLockDestroy(portdesc->rwlock_ptr, portm_port_free, portdesc);*/
  ...
  xTrace1(portmaintp, TR_EVENTS, "portmaint: port_kill: will unbind netport id %x", portdesc->net_port_number);
  ...
  if (portdesc->real_send_port != MACH_PORT_NULL) {
	...
	!= XK_SUCCESS)
      {
	xTrace2(portmaintp, TR_ERRORS, "port maint: cannot unbind local send port %x error code %x", portdesc->real_send_port, ret);
      }
    xTrace0(portmaintp, TR_ERRORS, "will deallocate local port");
    xTrace1(portmaintp, TR_ERRORS, "port maint: deallocating send right %x", portdesc->real_send_port);
    retcode = mach_port_mod_refs(mach_task_self(), portdesc->real_send_port, MACH_PORT_RIGHT_SEND, -1);
    if (retcode != KERN_SUCCESS) {
      retcode = mach_port_mod_refs(mach_task_self(), portdesc->real_send_port, MACH_PORT_RIGHT_SEND_ONCE, -1);
      if (retcode != KERN_SUCCESS)
	xTrace3(portmaintp, TR_ERRORS, "port maint: failed to destroy sendonce right %x ret = %d dec (%x)", portdesc->real_send_port, retcode, retcode);
    }
  }
  if (portdesc->real_receive_port != MACH_PORT_NULL) {
    if ((ret=mapUnbind(STATEPTR->maplocalports, &portdesc->real_receive_port))
	!= XK_SUCCESS)
      {
	xTrace2(portmaintp, TR_ERRORS, "port maint: cannot unbind local receive port %x error code %x", portdesc->real_receive_port, ret);
      }
    xTrace1(portmaintp, TR_ERRORS, "port maint: deallocating receive right %x", portdesc->real_receive_port);
    retcode = mach_port_destroy(mach_task_self(), portdesc->real_receive_port);
    if (retcode != KERN_SUCCESS)
      xTrace3(portmaintp, TR_ERRORS, "port maint: failed to destroy receive right %x ret = %d dec (%x)", portdesc->real_receive_port, retcode, retcode);
  }
  portm_port_free(NULL, portdesc);
}
...
{
  xTrace1(portmaintp, TR_EVENTS, "portmaintp: port_kill_mapfun called for portdesc ptr %x",
	  portdesc);
     ...
     IPhost *rebooted_host;
{
  ...
  xTrace1(portmaintp, TR_EVENTS, "portmaintp: port_kill_so_mapfun called for port %x",
	  port);

  if (IPEQUAL(receiver_host_addr, (*rebooted_host))) {
    if ((kret = mach_port_deallocate(mach_task_self(), port)) != KERN_SUCCESS)
      xTrace1(portmaintp, TR_ERRORS, "portmaintp:  port_kill_so_mapfun: port %x deallocation fails", port);
    return MFE_REMOVE | MFE_CONTINUE;
  }
  return MFE_CONTINUE;
}
...
portm_send_port_death(VOID *key, int mscount, VOID *arg)
{
  ...
  if (!(portd->current_read_desc && 
	(ls = portd->current_read_desc->lower_session) != ERR_XOBJ)){
    partInit(whom, 1);
     ...
     mach_port_t port;
{
  ...
  if ( (xret = mapResolve(STATEPTR->maplocalports, &port, &portd))
      ...
      (portd->net_port_rights &(MACH_PORT_TYPE_SEND | MACH_PORT_TYPE_SEND_ONCE)))
    {
      ...
      xTrace1(portmaintp, TR_EVENTS, "portmaint: portm_delete_local_send: Unbinding netport id %x", portd->net_port_number);
...
*/
      ...
      if (portd->lower_session != ERR_XOBJ) xClose(portd->lower_session);
      mapForEach(portd->session_map, (Pfi)portm_close_session, 0);
      portm_port_free(NULL, portd);
    }
  else {
     ...
     mach_port_t port;
{
  ...
  if (mapResolve(STATEPTR->maplocalports, &port, &portd) == XK_SUCCESS &&
      ...
      (portd->net_port_rights & MACH_PORT_TYPE_RECEIVE))
    {
      portd->net_port_type = MN_INVALID;
      /* don't let anyone else have this port */
      xTrace1(portmaintp, TR_EVENTS, "portm_delete_local_send: Unbinding netport id %x", portd->net_port_number);
      mapUnbind(STATEPTR->mapnetdesc, &portd->net_port_number);
      mapUnbind(STATEPTR->maplocalports, &portd->real_send_port);
      if (writerLock(portd->rwlock_ptr) != XK_SUCCESS) {
...
IPhost sender;
{
  ...
  mapBind(STATEPTR->mapsendonceports, &port, IP_TO_INT(sender));
  mapBind(STATEPTR->mapsendoncenumbers, &netport_id, port);
  return netport_id++;
}
  ...
  portm_fast_send_once(nnum, receiver) mnportid_t nnum; IPhost receiver;
{
  ...
  xTrace1(portmaintp, TR_EVENTS, "portmaint: portm_fast_send_once returning port %x", port);
  mapBind(STATEPTR->mapsendonceports, &port, IP_TO_INT(receiver));
  return port;
}
  ...
  portm_use_fast_send_once(port, dealloc) mach_port_t port; bool dealloc;
{
  ...
  xTrace1(portmaintp, TR_EVENTS, "portmaint: portm_use_fast_send_once freeing port %x", port);
  for (i=0; i<PORTM_SO_PORT_MAX; i++, portptr++)
 ...
 */
void
  portm_use_netnum(port, port_number)
  mach_port_t port;
  mnportid_t port_number;
{
  IPhost hostaddr;
  xkern_return_t xret;
  ...
  mapUnbind(STATEPTR->mapsendonceports, &port);
  mapUnbind(STATEPTR->mapsendoncenumbers, &port_number);
  if (xret == XK_SUCCESS) {
...
portmaint_init(self) XObj self;
{
  ...
  *myProtl = *machnetipcProtl;
  ...
  netport_id = NETPORT_INITIAL_NUMBER | (((int)ipaddr.d) << 24);
  xTrace2(portmaintp, TR_FUNCTIONAL_TRACE, "port_maint_init netport id %d (%x)",
	  netport_id, netport_id);

  /* establish communication with transport layer; same as machnetipc */
  partInit(whom, 1);
  ...
  STATESELFPTR->mapsendoncenumbers = mapCreate(2*PORTM_SO_PORT_MAX, sizeof(mnportid_t));
  ...
  /* this is a cache of receive rights to use for the server side of RPC's */
  for (i = 0; i<PORTM_SO_PORT_MAX; i++, portptr++) {
    kern_return_t kret;
...
xkern_return_t allocLocalRight( mnetport *portd, mach_port_type_t mtype )
{
  ...
  if (!portd->real_receive_port) {
	...
	!= KERN_SUCCESS) {
      xTrace0(portmaintp, TR_ERRORS, "portmaintp: allocLocalRight: could not allocate local receive port");
      return XK_FAILURE;
    }
  }

  /* bind the receive port to the netport number */
  if (mapBind( STATEPTR->maplocalports, (void *)&portd->real_receive_port,
		     (void *)portd) == ERR_BIND) {
    xTrace1(portmaintp, TR_ERRORS, "portmaintp: allocLocalRight: could not bind local receive port %x with netport", portd->real_receive_port);
    return XK_FAILURE;
  }  
  ...
  if (mtype == MACH_PORT_TYPE_RECEIVE)
    if ( !portd->real_send_port) {
      if ((ret = mach_port_extract_right(mach_task_self(),
					 portd->real_receive_port,
					 MACH_MSG_TYPE_MAKE_SEND,
					 &portd->real_send_port,
					 &returned_type))
	  != KERN_SUCCESS)
	{
	  xTrace3(portmaintp, TR_ERRORS, "portmaintp: allocLocalRight: could not extract send right from netport id %x port %x ret %x",
...
mach_port_t port; mnportid_t netnum;
{
  ...
  xTrace0(portmaintp, TR_FULL_TRACE, "portmaint: portm_move_netnum called");
  if( mapResolve(STATEPTR->mapsendonceports, &port, &receiver) == XK_SUCCESS){
   /* create new mnetport structure */
    return
      convert_to_netport(port, MACH_PORT_TYPE_SEND_ONCE, receiver, 0, netnum);
  }
  xTrace0(portmaintp, TR_ERRORS, "portmaint: could not find port in database");
  return Null_Netport;
}
 ...
 *   portm_move_receiver
 *
 *    respond to rrx protocol notification that receiver has moved
 *
 */
void
portm_move_receiver(portd) mnetport *portd;
{
  xTrace1(portmaintp, TR_FULL_TRACE, "portm_move_receiver called for port %x",
	  portd->net_port_number);
  portd->lower_session = ERR_XOBJ;  /* next send will open new session */
  portd->net_port_rights &= ~MACH_PORT_TYPE_RECEIVE;
/*  portd->receiver_host_addr = host; */
  if (portd->current_read_desc && portd->current_read_desc->blocked) {
    xTrace1(portmaintp, TR_EVENTS, "portm_move_receiver: Unblocking sender on port %x", portd->net_port_number);
    port_unblock(portd);
  }
}
