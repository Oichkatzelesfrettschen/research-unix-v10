...
kern_return_t ssr_init(XObj self);
int ssr_control(XObj self, int opcode, char *buf, int len);
static void connect_to_machr(Event ev, void *arg);
static void service_request_listen(void);

#else 

kern_return_t ssr_init();
int ssr_control();
...
static long
ssrdata_pop_and_swap(char *to, char *from, long len, enum SOURCE_BYTE_ARCH arch_type)
{
  struct ssrdata *ssr_data = (struct ssrdata *)to;
  int arch_index = arch_unpermute_index(arch_type);

  bcopy(from, (char *)&ssr_data->service_id, sizeof(int));
  ssr_data->service_id = unpermute_int32[arch_index]((char *)&ssr_data->service_id);
  from += sizeof(int);

  bcopy(from, (char *)&ssr_data->operation, sizeof(int));
  ssr_data->operation = unpermute_int32[arch_index]((char *)&ssr_data->operation);
  from += sizeof(int);

  bcopy(from, (char *)&ssr_data->sequence_num, sizeof(int));
  ssr_data->sequence_num = unpermute_int32[arch_index]((char *)&ssr_data->sequence_num);
  from += sizeof(int);

  bcopy(from, (char *)&ssr_data->destination_host, sizeof(IPhost));
  from += sizeof(IPhost);

  bcopy(from, (char *)&ssr_data->data_length, sizeof(int));
  ssr_data->data_length = unpermute_int32[arch_index]((char *)&ssr_data->data_length);
  from += sizeof(int);
...
mycopypop(char *to, char *from, long len, void *arg) {
  if ((int)len == len) {
    ...
    xTrace1(ssrp, TR_ERRORS, "Untenable len for copy function %ld", len);
  }
  return((long)0);
}
 ...
 */
static void
connect_to_machr(Event ev, void *arg)
{
  Part dummy[1];
...
ssr_process_user_request(struct local_ssr_msg *msg)
{
  ...
  switch (operation) {
    ...
    case PRIORITY:
      {
	int priority = service_id;

	xTrace1(ssrp, TR_MAJOR_EVENTS, "ssr: user request: set eth priority to %d", priority);
	xControl(xGetDown(myProtl, 0), ETH_SET_PRIORITY, (char *)&priority, sizeof(int));
      }
      break;
...
ssr_demux(XObj self_protocol, XObj lls, Msg *machdg)
{
    ...
    /* message is: reply_port, arch type, ssr data, user data
    ...
    msgPop(machdg, (Pfl)mycopypop, (void *)&arch_type, sizeof (enum SOURCE_BYTE_ARCH), (void *)0);
	...
	xTrace2(ssrp, TR_ERRORS, "ssr_demux: could not get port type for port %x failure code %x hex", mmsg.mmhdr.msgh_local_port, kret);
     ...
     xTrace2(ssrp, TR_DETAILED, "machripc: netmach_to_actualmach: got port right of type %x for reply port %x", local_port_type, mmsg.mmhdr.msgh_local_port);
   }
   if (local_port_type & MACH_PORT_TYPE_RECEIVE)
     extraction_type = MACH_MSG_TYPE_MAKE_SEND;

    msgPop(machdg, (Pfl)ssrdata_pop_and_swap, (void *)&regdata, sizeof(struct ssrdata), (void *)arch_type);
