 ...
 * HISTORY
 * $Log: xk_mms.c,v $
 * Revision 2.7.1.2  1993/04/08  17:39:07  menze
 * Added inclusion of proxy_util.h
 *
 * Revision 2.7.1.1  1993/03/23  22:58:42  menze
 * modified for x-kernel ... a new request buffer is allocated every time
 * and passed to the demux routine.  The demux routine is responsible for
 * eventually freeing the buffer.
 *
...
{
    register mig_reply_header_t *bufRequest, *bufReply;
    register mach_msg_return_t mr;

    xk_master_lock();
    bufRequest = (mig_reply_header_t *) xMalloc(max_size);
    if (bufRequest == 0)
	return KERN_RESOURCE_SHORTAGE;
    bufReply = (mig_reply_header_t *) xMalloc(max_size);
    if (bufReply == 0)
	return KERN_RESOURCE_SHORTAGE;
    xk_master_unlock();
