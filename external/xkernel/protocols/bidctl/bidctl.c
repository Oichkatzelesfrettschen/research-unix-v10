    ...
    int		create;
{
	...
	bs->bind = mapBind(ps->bsMap, peer, (VOID *)bs);
	if ( bs->bind == ERR_BIND ) {
	    /* 
	    ...
	    xTrace0(bidctlp, TR_SOFT_ERRORS,
		    "mapBind failed ... state must already exist");
	    bidctlDestroy(bs);
