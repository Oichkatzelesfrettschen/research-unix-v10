 ...
 *
 * $Revision: 1.7 $
 * $Date: 1993/04/30 23:39:59 $
 */

/* 
 * Test protocol to exercise IP routing in a rudimentary fashion.
 * Grabs IP by name, so it doesn't need to be configured above
 * anything (should be initialized after IP, though.)
 *
 * This test will not actually send any packets, but it does need to
 * be able to get an ethernet address certain hosts:
 *
 * 	localPeer
 *	Gateway in route1
 *	Gateway in route1_2
 *	Gateway in route2
 *
 * Either run this on a network where such hosts exist or modify
 * your rom file appropriately.
 */
...
{
    partInit(p, 1);
    partPush(*p, remHost, sizeof(IPhost));
}
