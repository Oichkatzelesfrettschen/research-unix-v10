...
#define ERROR_RETURN { rep->replyCode = SRX_FAILURE;	\
     ...
     * bookkeeping easier.
     */
    xTrace1(srxp, TR_EVENTS, "SRX Getting a WRITER LOCK on port %x", npd->net_port_number);
    ...
    VOID	*arg;
{
    xTrace2(srxp, TR_DETAILED,
	    ...
	    np->net_port_number, ipHostStr((IPhost *)arg));
    xFree((char *)arg);
    xTrace1(srxp, TR_EVENTS, "SRX Releasing a WRITER LOCK on port %x", np->net_port_number);
    ...
    SrxRepMsg	*rep;
{
	...
	if ( findNetPort(&netport, req->archTag, TRUE, &npd)
	    == XK_FAILURE ) {
	    xError("receiveNewSendRight -- findNetPort(create) fails!");
	    rep->replyCode = SRX_FAILURE;
	    return;
	}
	updatePortId(npd, req->u.xfer.msgId, req->u.xfer.rr.h);
    }
