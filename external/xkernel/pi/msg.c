...
/* this compensates for a bug on the Mach platform */
#define KLUDGE 4
...
static bool msgInternalForEach( MNode *, long, long, XCharFun, VOID *);
#ifdef MSG_NR_TEST
bool msgnrUserFun(MNode *this, long off, long len, VOID *arg);
void msgnrtest(void);
#endif MSG_NR_TEST
...
void msgDestroy();
void msgJoin();
static void msgInternalShow( );
static bool msgInternalForEach( );
#ifdef MSG_NR_TEST
bool msgnrUserFun();
void msgnrtest();
#endif MSG_NR_TEST
...
#if defined(__STDC__) || defined(__GNUC__)
static void msgnrstackinit(struct mstack *);
/* 
 * types of user functions called in ForEach()
 */
typedef bool (*XNodeFun)( struct mstack *, MNodePair *, void * );
typedef bool (*XLeafFun)( MNode *, void *);

#else
static void msgnrstackinit();
typedef bool (*XNodeFun)();
typedef bool (*XLeafFun)();
#endif
    ...
    MNode *this;
{
  ...
  if (atomicDec(&this->refCnt) == 1) {
    ...
    case t_MNodePage:
      ...
      break;
    case t_MNodeUsrPage:
      xTrace3(msg, TR_FULL_TRACE, "decRef t_MNodeUsrPage %x %x %d", this->b.usrpage.bFree,
...
#if defined(__STDC__) || defined(__GNUC__)
static void msgnrpush(struct mstack *, MNodePage *);
static void msgnrstackreinit(struct mstack *);
static bool msgnrNodeDestroy( struct mstack *, MNodePair *, void  *);
static void msgNodeForEach( MNode *, XNodeFun, XLeafFun, VOID *,  struct mstack *);
#else
static void msgnrpush();
static void msgnrstackreinit();
static bool msgnrNodeDestroy( );
static void msgNodeForEach( );
#endif

static bool decRefnr(this)
    MNode *this;
{
  ...
  if (atomicDec(&this->refCnt) == 1) {
    ...
    case t_MNodePage:
      ...
      break;
    case t_MNodeUsrPage:
      xTrace3(msg, TR_FULL_TRACE, "decRefnr t_MNodeUsrPage %x %x %d", this->b.usrpage.bFree,
	      this->b.usrpage.data, this->b.usrpage.size);
      this->b.usrpage.bFree(this->b.usrpage.data, this->b.usrpage.size);
      FREEONE(mup, this);
      break;
    case t_MNodeBuf:
      xTrace2(msg, TR_FULL_TRACE, "decRefnr t_MNodeBuf %x %x", this->b.buf.bFree,
	      this->b.buf.data);
      this->b.buf.bFree(this->b.buf.data);
      FREEONE(mbf, this);
      break;
...
/* creator/constructor */
static MNodePage *
newSmallMNodePage(bytes)
    int	bytes;
{
  MNodePage *this;

  xTrace1(msg, TR_FULL_TRACE, "newSmallMNodePage %d bytes", bytes);

#ifdef MSG_STATISTICS
  msgSpace_used += bytes;
#endif MSG_STATISTICS

  this = (MNodePage *)xMalloc(bytes);
  this->refCnt = 1;
  this->type = mpgtype;
  RECORD_USAGE(mpg);
  xTrace1(msg, TR_MAJOR_EVENTS, "msgtool: small page %x", this); 

  this->b.page.size = bytes - MNodeLeafSize - KLUDGE;
  this->b.page.data = this->b.page.buffer;
    ...
    char **buf;
{
  ...
  xAssert(len > 0);
  if (roundlen+MNodeLeafSize+sizeof(int)+KLUDGE < PageSize/2) {
    int bytes = PageSize/2;
    while (bytes/2 > roundlen+MNodeLeafSize+sizeof(int)+KLUDGE) bytes /= 2;
    this->tree = this->stack = newSmallMNodePage(ROUND4(bytes-sizeof(int)));
    xTrace1(msg, TR_MAJOR_EVENTS, "msgConstructAllocate small page %d", roundlen);
  }
  else
    this->tree = this->stack = newMNodePage(PAGEROUND(roundlen));
#ifdef MSG_NEW_ALG
 ...
 *
 */
static void msgnrstackinit(stack)
     struct mstack *stack;
{
  xTrace0(msg, TR_FULL_TRACE, "msgnrstackinit");

  stack->size = MSG_NR_STK_SZ;
  stack->top = stack->bottom = (MNodePage **)xMalloc(MSG_NR_STK_SZ);
  stack->limit = (MNodePage **)(((char *)(stack->bottom)) + MSG_NR_STK_SZ);
}

static void msgnrpush(stack, item)
     struct mstack *stack;
     MNodePage *item;
{

  xTrace0(msg, TR_FULL_TRACE, "msgnrpush");
  ...
  *stack->top++ = item;
  if (stack->top >= stack->limit) {
    ...
    stack->size = newsize;
    stack->limit = (MNodePage **)(((char *)(stack->bottom)) + stack->size);
    xTrace1(msg, TR_SOFT_ERROR, "msgnrpush overflow to %d", stack->size);
  }
}

static bool msgnrpop(stack, item)
     struct mstack *stack;
     MNodePage **item;
{
  xTrace0(msg, TR_FULL_TRACE, "msgnrpop");
  if (--(stack->top) < stack->bottom) {
    xTrace0(msg, TR_SOFT_ERROR, "msgnrpop underflow");
    return FALSE;
  }
  *item = *(stack->top);
  return TRUE;
...
static void msgNodeForEach(this, fnode, fleaf, arg, stack)
    ...
    struct mstack *stack;
{
  bool stackcontinue = TRUE;
  bool nodecontinue = TRUE;
 ...
 */
static bool msgnrNodeDestroy(stack, this, arg)
    struct mstack *stack;
    MNodePair *this;
    void  *arg;
{
  ...
  if (atomicDec(&this->refCnt) == 1)
    if (this->type == t_MNodePair) {
      if (this->b.pair.l.node) msgnrpush(stack, this->b.pair.l.node);
      if (this->b.pair.r.node) msgnrpush(stack, this->b.pair.r.node);
      FREEONE(mpr, this);
    }
     ...
     Msg *this;
{
  ...
  msgnrstackreinit(&msgStack);
  msgNodeForEach(this->tree, msgnrNodeDestroy, decRefnr, 0, &msgStack);
}

#ifdef MSG_NR_TEST

bool msgnrUserFun(this, off, len, arg)
     MNode *this;
     long off, len;
     VOID *arg;
{
 ...
 */
#if defined(__STDC__) || defined(__GNUC__)
typedef bool (*XNodeFunArgs)( struct mstack *, MNode *, long, long );
typedef bool (*XLeafFunArgs)( MNode *, long, long);
static bool	msgnrPairArgs( struct mstack *, MNodePair *, long, long );
static bool	msgnrLeafShow( MNode *, long, long );

#else

typedef bool (*XNodeFunArgs)();
typedef bool (*XLeafFunArgs)();
static bool	msgnrPairArgs( );
static bool	msgnrLeafShow( );
#endif


static int msgindent;
static XCharFun msgfunction;
static void *msgarg;
...
static bool msgnrPairArgs(stack, this, off, len)
    MNodePair *this;
    struct mstack *stack;
    long off, len;
{
...
static bool msgnrLeafShow(this, off, len)
    MNode *this;
    long off, len;
{
  int lim, i;
...
static void msgNonRecForEach(this, fnode, fleaf, arg1, arg2, stack)
    MNode *this;
    XNodeFunArgs fnode;
    XLeafFunArgs fleaf;
    long arg1, arg2;
    ...
    Msg *this;
{
  ...
  msgindent = 0L;
  if (length > 0)
    (void) msgNonRecForEach(this->tree, msgnrPairArgs, msgnrLeafShow, offset, length, &msgStack);
#else
  if (this->length > 0)
    (void) msgNonRecForEach(this->tree, msgnrPairArgs, msgnrLeafShow, this->offset, this->length, &msgStack);
#endif MSG_NEW_ALG
}

static bool msgnrLeafForEach(this, off, len)
     MNode *this;
     long off, len;
{
  ...
  xAssert(this->b.leaf.size - off >= len);
#ifdef MSG_NEW_ALG
  if (msgfunction==tu_leaf) return(tu_leaf(this, off, len, msgarg));
  else
#endif MSG_NEW_ALG
    return (msgfunction)(this->b.leaf.data + off, len, msgarg);
}
    ...
    VOID *arg;
{
  ...
  msgarg = arg;
#ifdef MSG_NEW_ALG
  (void) msgNonRecForEach(this->tree, msgnrPairArgs, msgnrLeafForEach, offset, length, &msgStack);
#else
  (void) msgNonRecForEach(this->tree, msgnrPairArgs, msgnrLeafForEach, this->offset, this->length, &msgStack);
#endif MSG_NEW_ALG
}
