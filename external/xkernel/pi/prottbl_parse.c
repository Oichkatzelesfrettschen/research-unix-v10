...
ptblRomOpt( str, nFields, ln, arg )
    char	**str;
    int		nFields;
    int		ln;
    VOID	*arg;
{
    if ( protTblBuild(str[1]) ) {
	sprintf(errBuf, "error building protocol table %s", str[1]);
	xError(errBuf);
	return XK_FAILURE;
    }
    return XK_SUCCESS;
